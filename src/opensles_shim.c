...
static const int id_seek_tag = 8;
static const int id_pitch_tag = 9;
static const int id_ratecontrol_tag = 10;
static const int id_presetreverb_tag = 11;
static const int id_playerevent_tag = 12;
static const int id_metadata_tag = 13;
static const int id_3dlocation_tag = 14;
static const int id_3ddoppler_tag = 15;
static const int id_3dsource_tag = 16;
static const int id_dynamicsource_tag = 17;
static const int id_dynamicinterface_tag = 18;
static const int id_mutesolo_tag = 19;
static const int id_bassboost_tag = 20;
static const int id_equalizer_tag = 21;
static const int id_virtualizer_tag = 22;
static const int id_outputmix_tag = 23;
static const int id_audioiodev_tag = 24;
...
const SLInterfaceID sl_IID_OUTPUTMIX = &id_outputmix_tag;
const SLInterfaceID sl_IID_AUDIOIODEVICECAPABILITIES = &id_audioiodev_tag;
const SLInterfaceID sl_IID_PITCH = &id_pitch_tag;
const SLInterfaceID sl_IID_RATECONTROL = &id_ratecontrol_tag;
const SLInterfaceID sl_IID_PRESETREVERB = &id_presetreverb_tag;
const SLInterfaceID sl_IID_PLAYEREVENT = &id_playerevent_tag;
const SLInterfaceID sl_IID_METADATAEXTRACTION = &id_metadata_tag;
const SLInterfaceID sl_IID_3DLOCATION = &id_3dlocation_tag;
const SLInterfaceID sl_IID_3DDOPPLER = &id_3ddoppler_tag;
const SLInterfaceID sl_IID_3DSOURCE = &id_3dsource_tag;
const SLInterfaceID sl_IID_DYNAMICSOURCE = &id_dynamicsource_tag;
const SLInterfaceID sl_IID_DYNAMICINTERFACEMANAGEMENT = &id_dynamicinterface_tag;
const SLInterfaceID sl_IID_MUTESOLO = &id_mutesolo_tag;
const SLInterfaceID sl_IID_BASSBOOST = &id_bassboost_tag;
const SLInterfaceID sl_IID_EQUALIZER = &id_equalizer_tag;
const SLInterfaceID sl_IID_VIRTUALIZER = &id_virtualizer_tag;
...
typedef struct {
  ...
  void *play_vtable[16];
  void *play_ptr;
  void *volume_vtable[16];
  void *volume_ptr;
  void *bq_vtable[16];
  void *bq_ptr;
  void *effectsend_vtable[16];
  void *effectsend_ptr;
} AudioPlayer;
...
static SLresult play_SetPositionUpdatePeriod(void *self, SLmillisecond mSec) {
  (void)self; (void)mSec;
  return SL_RESULT_SUCCESS;
}

static SLresult play_GetPositionUpdatePeriod(void *self, SLmillisecond *pMSec) {
  (void)self;
  if (pMSec) *pMSec = 0;
  return SL_RESULT_SUCCESS;
}
...
static SLresult bq_Enqueue(void *self, const void *pBuffer, SLuint32 size) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].bq_ptr == itf_ptr) {
      ...
      /* Enfileira no ring buffer lock-free (SPSC: caller escreve, SDL callback lê) */
      uint32_t writable = ring_writable(p);
      if (size > writable) {
        /* Underrun / overflow do ring -- não cabe tudo, descarta o que não couber */
        debugPrintf("opensles_shim: WARNING: ring buffer overflow (want %u, free %u)\n",
                    size, writable);
        size = writable;
      }
      if (size > 0) {
        ring_write(p, pBuffer, size);
        queue_push(p, size);
      }
      return SL_RESULT_SUCCESS;
    }
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].bq_ptr == itf_ptr) {
      ...
      if (pState) {
        ...
        state[1] = p->played_bytes / (p->num_channels * (p->bits_per_sample / 8));
      }
      return SL_RESULT_SUCCESS;
    }
  }
...
static SLresult bq_GetState_or_RegisterCallback(void *self, void *a1, void *a2) {
  uintptr_t code = (uintptr_t)a1;
  int is_code_ptr = (text_base && text_size &&
                     code >= (uintptr_t)text_base &&
                     code < (uintptr_t)text_base + text_size);
  if (is_code_ptr) {
    return bq_RegisterCallback(self, (slBufferQueueCallback)a1, a2);
  } else {
    return bq_GetState(self, a1);
  }
}
...
static SLresult player_GetInterface(void *self, SLInterfaceID iid, void **pInterface) {
  if (!pInterface) return SL_RESULT_SUCCESS;
  void **itf_ptr = (void **)self;
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].obj_ptr == itf_ptr) {
      AudioPlayer *p = &g_players[i];
      if (iid == sl_IID_PLAY) {
      ...
      if (iid == sl_IID_EFFECTSEND) {
        *pInterface = &p->effectsend_ptr;
        return SL_RESULT_SUCCESS;
      }
      static void *stub_vtable[16];
      static void *stub_ptr;
      static int inited = 0;
      if (!inited) {
        for (int k = 0; k < 16; k++) stub_vtable[k] = (void *)stub_success;
...
static void setup_player_vtables(AudioPlayer *p) {
  for (int i = 0; i < 16; i++) p->obj_vtable[i] = (void *)stub_success;
  ...
  for (int i = 0; i < 16; i++) p->play_vtable[i] = (void *)stub_success;
  ...
  p->play_vtable[10] = (void *)play_SetPositionUpdatePeriod;
  p->play_vtable[11] = (void *)play_GetPositionUpdatePeriod;
  p->play_ptr = p->play_vtable;

  for (int i = 0; i < 16; i++) p->volume_vtable[i] = (void *)stub_success;
  ...
  for (int i = 0; i < 16; i++) p->bq_vtable[i] = (void *)stub_success;
  ...
  for (int i = 0; i < 16; i++) p->effectsend_vtable[i] = (void *)stub_success;
  p->effectsend_ptr = p->effectsend_vtable;
}

/* Output mix */
static void *g_outmix_vtable[16];
...
static SLresult outmix_GetInterface(void *self, SLInterfaceID iid, void **pInterface) {
  ...
  static int inited = 0;
  if (!inited) {
    for (int i = 0; i < 16; i++) stub_vtable[i] = (void *)stub_success;
    stub_ptr = stub_vtable;
    inited = 1;
  }
...
static void init_outmix(void) {
  ...
  if (inited) return;
  inited = 1;
  for (int i = 0; i < 16; i++) g_outmix_vtable[i] = (void *)stub_success;
...
/* Engine object */
static void *g_engine_obj_vtable[16];
...
static void init_engine(void) {
  ...
  for (int i = 0; i < 16; i++) g_engine_obj_vtable[i] = (void *)stub_success;

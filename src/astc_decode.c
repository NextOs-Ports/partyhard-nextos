...
static int astc_load_library(void)
{
    ...
    if (!handle) {
        fprintf(stderr, "[st/astc] cannot load libastcUtil.so: %s\n",
                dlerror());
        g_load_state = -1;
        return -1;
    }
    ...
    if (!p_config_init || !p_context_alloc || !p_decompress_image) {
        fprintf(stderr,
                "[st/astc] missing symbols cfg=%p ctx=%p dec=%p reset=%p\n",
    ...
    g_load_state = 1;
    fprintf(stderr, "[st/astc] Arm astcenc decoder loaded\n");
    return 0;
}
...
static astcenc_context *astc_get_context(int block_x, int block_y)
{
    ...
    if (result) {
        fprintf(stderr, "[st/astc] config %dx%d failed: %d\n",
    ...
    if (result || !context) {
        fprintf(stderr, "[st/astc] context %dx%d failed: %d\n",
                block_x, block_y, result);
        return NULL;
    }
    g_contexts[block_x][block_y] = context;
    fprintf(stderr, "[st/astc] context %dx%d ready\n", block_x, block_y);
    return context;
}
                     ...
                     int height, int block_x, int block_y)
{
    ...
    if (result) {
        fprintf(stderr, "[st/astc] decode %dx%d/%dx%d failed: %d\n",

...
/* Painel fisico (drawable real); igual a screen_* fora do `stretch`. */
static int panel_width = 1280;
static int panel_height = 720;
...
static int create_window(void)
{
    ...
    for (size_t i = 0; i < sizeof formats / sizeof formats[0]; i++) {
        ...
        video_window = SDL_CreateWindow(
            "Party Hard GO", SDL_WINDOWPOS_CENTERED,
    ...
    /* Politica `stretch` declarada pelo port (nxproject display.policy): num
     * painel que nao e' 16:9 a Unity passa a enxergar uma superficie interna
     * 16:9 e o present estica-a para o painel inteiro (sem barras). */
    {
        int iw = 0, ih = 0;
        if (st_stretch_setup(panel_width, panel_height, &iw, &ih)) {
            screen_width = iw;
            screen_height = ih;
            extern void st_window_set_size(int, int);
            st_window_set_size(iw, ih);
        }
    }
    ...
    nxgl_frame_proof_set_video_context(
        ...
        get_string && get_string(GL_VERSION)
            ? (const char *)get_string(GL_VERSION) : NULL);
    st_graphics_contract_set_sdl_window(video_window);
                                     ...
                                     EGLSurface read, EGLContext handle)
{
    ...
    if (surface->is_window)
        st_stretch_after_present(); /* o "0" da engine e' o alvo interno */
    if (surface->is_window &&
        !st_graphics_contract_pre_present((uintptr_t)context->id))
...
EGLBoolean st_sdl_swap_buffers(EGLDisplay display, EGLSurface handle)
{
    ...
    if (!current_context || current_context->draw != handle ||
        ...
        return EGL_TRUE;
    /* A seta do ponteiro entra aqui, no swap da JANELA de verdade -- depois
     * dos descartes de pbuffer/superficie errada e ANTES da captura, para que
     * a prova por medicao enxergue exatamente o que a tela recebe. */
    /* `stretch`: o alvo interno vai para o painel antes da seta. */
    st_stretch_present();
    {
        extern void st_draw_gamepad_cursor(void);
        st_draw_gamepad_cursor();
    }
    capture_frame_if_requested();
    /* Fronteira real de present do caminho SDL: a prova amostra o quadro
     * que a tela recebe (seta inclusa) imediatamente antes do swap. */
    nxgl_frame_proof_before_present(panel_width, panel_height);
    SDL_GL_SwapWindow(video_window);
    if (!st_graphics_contract_after_present(
...
static const char *sdl_eglQueryString(EGLDisplay display, EGLint name)
{
    (void)display;
    switch (name) {
    case EGL_VENDOR:      return "Party Hard GO SDL EGL";
